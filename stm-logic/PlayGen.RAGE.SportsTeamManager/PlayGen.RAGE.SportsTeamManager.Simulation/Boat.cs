	...
	public class Boat
	{
		...
		internal void GetIdealCrew(Dictionary<string, CrewMember> crewMembers, string managerName)
		{
			...
			foreach (var possibleCrew in crewCombos)
			{
				...
				if (opinionTotal > opinionMax)
				{
					opinionMax = opinionTotal;
				}
			}
			var bestScore = 0;
			...
			foreach (var possibleCrew in crewCombos)
			{
				...
				if (crewPositionScores.Values.Take(positionNames.Count).Sum() + opinionMax >= bestScore)
				{
					...
					if (possibleCrew.Select(pc => crewPositionScores.First(cps => cps.Key.Crew == pc).Value).Sum() + opinionScore < bestScore)
					{
						continue;
					}
